...
struct VehicleParams {
    ...
    // Hull volume tuned for ~near-neutral buoyancy (slightly positive).
    static inline VehicleParams reference_torpedo() {
        VehicleParams v;
        v.name = "ref-torpedo";

        // Hull defaults are fine; tweak volume for slight positive buoyancy.
        v.hull.mass_kg = 25.0;
        v.hull.volume_m3 = 25.0 / 1025.0 * 1.002; // ~0.2% positively buoyant

        Thruster main_aft;
        main_aft.position_b = {-0.50, 0.00, 0.00};
        main_aft.axis_b = {1.0, 0.0, 0.0};
        main_aft.max_thrust_N = 35.0;
        v.thrusters.push_back(main_aft);

        Thruster bow_vert;
        bow_vert.position_b = {0.35, 0.00, 0.00};
        bow_vert.axis_b = {0.0, 0.0, 1.0};
        bow_vert.max_thrust_N = 15.0;
        v.thrusters.push_back(bow_vert);

        Thruster stern_vert;
        stern_vert.position_b = {-0.40, 0.00, 0.00};
        stern_vert.axis_b = {0.0, 0.0, 1.0};
        stern_vert.max_thrust_N = 15.0;
        v.thrusters.push_back(stern_vert);

        Thruster stern_lat;
        stern_lat.position_b = {-0.45, 0.00, 0.00};
        stern_lat.axis_b = {0.0, 1.0, 0.0};
        stern_lat.max_thrust_N = 12.0;
